...
When automating the ARC-1 CLI, parse the returned MCP envelope as well as the process status: a tool failure can be returned as JSON with `isError: true` while the CLI process still exits with status 0. Treat that envelope as a failed gate and never continue to activation. `SAP_ALLOWED_PACKAGES` is the canonical allowlist variable, and a long-running MCP process does not acquire later environment/configuration changes; restart it or use a bounded fresh process after an allowlist change, then verify the target system before writing.
...
- When the same grammar feature is recognized at more than one stage (for example parser, authorization scanner, and generator sink), share the predicate/normalization where possible and add a full-path regression through every stage. Exercise tabs, line breaks, and repeated spaces; a parser-only test does not prove that the executable representation selects the same path.
...
- Scope browser interaction to the editor control. After ALV renders, grid cells also expose textbox roles, so an unscoped textbox locator is ambiguous and must not be used as evidence of an editor action.
- For an error-path UI smoke, record what independently proves that the intended parser/compiler/runtime boundary was reached. The generic user message and an unchanged dump set prove safe presentation, not boundary reachability by themselves.
