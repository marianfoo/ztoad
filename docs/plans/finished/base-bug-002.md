...
## Post-CI process audit

- The first refreshed PR run was green for repository Quality and required abaplint checks.
- Sequential integration, red replay, exact-source hashing, explicit activation, live Unit, ATC, marked WebGUI/ST22, master restoration, and native-abapGit verification all contributed independent evidence; none should be removed.
- Existing guidance already required the dump marker before browser smoke and editor-scoped locators after ALV. The final evidence repeated the smoke when the marker was initially missing and narrowed the F3 action to the TextEdit textarea when grid cells made an unscoped textbox locator ambiguous.
- The missing rule was ABAP fixed-length character blank preservation. `AGENTS.md` and `docs/test-strategy.md` now require a live whitespace-preservation regression for tokenizers/maskers rather than treating static lint as behavioral proof.
- No CI workflow change is needed: the repository gate remained fast and green, while the live SAP gate correctly caught behavior that the local syntax model cannot execute.
