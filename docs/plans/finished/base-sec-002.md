...
- First PR CI: PR #18 passed required `Repository quality (ABAP 7.50)` and external abaplint checks.
- Post-green audit: repository settings now permit squash merge only after accidental merge commits on earlier PRs exposed workflow commits. `AGENTS.md`, `docs/development.md`, and the PR template now require squash merge. The guidance also records the observed large-report stdin `EAGAIN` recovery and complete-result ATC summarization requirement. The post-audit run `31103159862` passed repository quality and abaplint; a final documentation-only run verifies this evidence update.
