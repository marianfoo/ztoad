...
6. Run `npm ci`, `npm test`, `git diff --check`, and the final local/security review. Commit and freeze the exact source/object candidate before the expensive live gates, and record its commit plus source hash. Any later `src/` or serialized-object change invalidates the affected live evidence and must be redeployed and rechecked.
